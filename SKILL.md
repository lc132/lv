- **v6.16.14**: 回测交易明细修复——(1)MD报告"最近交易明细"从简单top20改为按日期均匀采样(每日期最多10条) (2)HTML报告同理改为每日期最多15条 (3)综合指标新增样本日期范围行 (4)解决多日样本时只显示最晚一天的问题，17日+21日数据现在均可见。
...
description: A股每日盘前短线标的智能筛选(v6.16.14)。基于前一日收盘数据，通过37步筛选流程，输出短线标的_YYYYMMDD.md和可视化HTML报告。同策略+跨策略冠军PK采用基本面+技术面融合7维度。回测报告新增👑皇冠回测板块+按日期均匀采样交易明细。
---
# A股盘前短线标的筛选 v6.16.14
