...
**B超跌反弹(优2)**：连跌≥3日、量<5日均×0.6、RSI(14)连续≥3日<35或底背离、MA20/MA60支撑、KDJ的K<20且J拐头向上（增强B最低置信★★）；**反弹确认：收盘站上MA5+成交量>昨日×1.2**；**趋势底线：股价<MA60→跳过策略B**；仓位：强10-12%/震荡12-15%/弱12-15%。**v6.6.42最大跌幅限制**：跌幅>30%→排除（`_excluded_max_decline`标记），深度跌幅可能暴雷，不做反弹。
...
**D回调企稳突破(优4)**：20日内创新高+回调至MA20±3%+连续3日缩量+站回MA5放量(量>昨日×1.3)；仓位：强10-12%/震荡12-15%/弱8-12%；D与A不能同时匹配，以A优先。D与B同时匹配→D优先。**v6.6.42板块联动增强**：同一板块≥3只A/D策略标的→该板块所有A/D标的得分+10%（`_tie_score`加0.10）。
...
**加分项**：板块TOP5+1、信号加分项(首阴候选+1)、K线形态确认+1。**v6.6.42增强**：均线粘合发散（振幅<3%+量比>1.2→`_tie_score`+0.05）+板块联动（同板块≥3只A/D→`_tie_score`+0.10）。
**参考项加分**：ROE>15%(+2分) + ROE 5-15%(+1分) + ROE<0%(-1分) + 经营现金流为正(+1分)。
**L3扣分**：L3级信号触发→减2分（主力净流出/融券增/行业利空）。
**v6.6.42新增扣分**：A策略假突破(`_fake_breakout`)→减3分；C策略量价不足(`_c_downgrade`)→减2分。
置信：≥9★★★ | 6-8★★ | <6★。分值为整数，如遇小数→向上取整判定置信度（如5.2→★★，8.8→★★★）。策略冲突按优先级归类，动量+超跌同时→以动量为准。**v6.6.42**新增F策略优先级(优6)，E与F互斥→E优先。
