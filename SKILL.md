...
description: A股每日盘前短线标的智能筛选(v6.6.26)。基于前一日收盘数据，通过 35步筛选流程（网络授时北京时间→GitHub拉取持仓跟踪→节假日检查→极端行情→外围市场→持仓同步→做T评估→持仓跟踪同步→持仓危机检查→全市场API拉取(东方财富clist→新浪→pytdx三级降级)→板块/代码段行业查表补全→31项硬排除(L1/L2/L3三级可达性)→14项信号过滤→五大策略评分（含评分相同时二次评估打破平局）→行业集中度→新闻筛查→生成HTML报告→GitHub同步(含超15天旧文件自动清理)→飞书推送→每周复盘+数据源监控告警(连续10次clist不可达则直达用户），输出短线标的_YYYYMMDD.md 预测次日上涨的标的为Markdown格式，同时生成可视化HTML报告。推荐历史按日期归档(推荐历史_YYYYMMDD.json)互不覆盖，告警日志仅在自动化中写。当用户需要运行盘前筛选、A股短线选股、每日标的预测时使用。
---
# A股盘前短线标的筛选 v6.6.26
...
### 数据源三级降级体系（v6.6.24 重写）

行情采集按优先级依次尝试三个数据源，不可达时自动降级：

| 优先级 | 数据源 | 方式 | 覆盖字段 | 失败处理 |
|--------|--------|------|---------|----------|
| **Tier 1** | 东方财富 clist | HTTP API | 全部 15 字段（含主力净流入） | → 降级 Tier 2 |
| **Tier 2** | 新浪财经 | HTTP 批量请求 | 仅 OHLCV（无量比/换手率/主力流入） | → 降级 Tier 3 |
...
| 字段 | Tier1(clist) | Tier2(Sina) | Tier3(pytdx) |
|------|:---:|:---:|:---:|
...
**Tier 1 (clist)** — 完整条件，无降级：
- A: 涨幅3-7% + 量比1.5-3.0 + 量>5日均×1.5 + MA5>MA10>MA20（K线需单独获取）
- B: 连跌≥3日 + RSI<35 + KDJ J拐头 + MA20/MA60支撑
- D: 20日内新高 + 回调至MA20±3% + 缩量站回MA5放量
- E: 北向连续净买 + 主力流入>{northbound_threshold}万 + 涨幅<2%

**Tier 2 (Sina)** — 代理指标（v6.6.22）：
| 原始指标 | 新浪代理 | 说明 |
|---------|---------|------|
| 量比 | 成交额分位数 | P75=最活跃25% |
...
- A: 涨幅3-7% + 量比1.5-3.0 + MA5>MA10>MA20（恢复到 clist 全条件，仅缺主力流入）
...
| 字段 | 说明 |
|------|------|
| `clist_success` | clist 累计成功次数 |
| `clist_consecutive_failures` | clist 连续失败次数（成功即归零） |
| `total_runs` | 总运行次数 |
| `last_source` | 最近一次成功的数据源 |
...
```json
{
  ...
  "history": [
    {"date": "2026-06-12", "source": "clist", "count": 5127, "success": true},
    {"date": "2026-06-11", "source": "sina", "count": 4892, "success": true, "reason": "clist API不可达(网络超时)"}
  ]
}
```
...
**搜集行情（步骤10）**：按三级优先级拉取全市场行情数据。Tier 1 → 东方财富 clist API（`push2.eastmoney.com/api/qt/clist/get`，全市场约5000+只，含主力净流入）；不可达 → Tier 2 新浪批量API（`hq.sinajs.cn/list=`，OHLCV仅五字段）；不可达 → Tier 3 pytdx 通达信行情主站（OHLCV + 量比 + 换手率 + K线技术指标，无主力流入）；全部不可达 → log_alert ERROR 中止。
...
**步骤10B — 板块/行业补全**（v6.6.23 重写：代码段查表替代「未知」兜底，杜绝板块冒充行业）：

当 clist API 或新浪 API 无法返回申万行业分类时，使用以下优先级查表策略：

**优先级1 — 个股精准覆盖表**（v6.6.26 新增，修复代码段查表偏差）：先查此表，命中直接返回，不进入优先级2。

```python
# 个股精准覆盖表（v6.6.26）— 修正代码段查表偏差
COMPANY_OVERRIDE = {
    # 600000-600099 段被映射为银行，实际为其他行业
    '600006': '汽车',      # 东风汽车
    '600031': '机械设备',  # 三一重工
    '600050': '通信',      # 中国联通
    '600066': '汽车',      # 宇通客车
    '600085': '医药生物',  # 同仁堂
    # 603900-603999 段被映射为商贸零售，实际为其他行业
    '603979': '有色金属',  # 金诚信(矿业服务)
    '603993': '有色金属',  # 洛阳钼业
    # 600200-600299 段被映射为医药生物，实际为其他行业
    '600276': '医药生物',  # 恒瑞医药(正确)
    '600298': '食品饮料',  # 安琪酵母
    # 可扩展更多个股修正
}
...
**优先级2 — 代码段查表**（v6.6.23，覆盖约85%标的）：优先级1未命中时执行。

```python
# 代码段 → 申万一级行业查表 (v6.6.26)
INDUSTRY_MAP = {
    # 上海主板 600xxx
    '600000-600099': '银行',    '600100-600199': '电子',
    '600200-600299': '医药生物', '600300-600399': '基础化工',
    '600400-600499': '电力设备', '600500-600599': '公用事业',
    '600600-600699': '食品饮料', '600700-600799': '交通运输',
    '600800-600899': '机械设备', '600900-600999': '银行',
    # 上海主板 601xxx
    '601000-601099': '非银金融', '601100-601199': '有色金属',
    '601200-601299': '非银金融', '601300-601399': '机械设备',
    '601400-601499': '银行',    '601500-601599': '非银金融',
    '601600-601699': '有色金属', '601700-601799': '电力设备',
    ...
    '001000-001099': '电子',     '001200-001299': '基础化工',
    '001300-001399': '机械设备',
...
# 查表逻辑：先查 COMPANY_OVERRIDE（优先级1），命中直接返回
# 未命中 → 查 INDUSTRY_MAP（优先级2）：code_int = int(code)；遍历区间，落在区间内即返回对应行业
# 两表均未覆盖 → industry = "未知"（绝不填入板块名）
```

**变更理由**：v6.6.22 及之前版本在行业不可得时以板块（深圳主板/上海主板）作为兜底值，导致行业列显示的是交易所板块而非申万行业。v6.6.23 改用代码段查表，查不到时标记「未知」，彻底杜绝分类学错误。v6.6.26 新增个股精准覆盖表（COMPANY_OVERRIDE），优先于代码段查表，修正代码段边界偏差（如600066宇通客车被误判为银行、603979金诚信被误判为商贸零售）。
