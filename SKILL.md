...
description: A股每日盘前短线标的智能筛选(v6.29.6)。基于前一日收盘数据，通过37步筛选流程+自动整改+策略级胜率监控熔断，输出短线标的_YYYYMMDD.md和可视化HTML报告。同策略+跨策略冠军PK采用基本面+技术面融合12维度。回测报告新增👑皇冠回测板块+按日期均匀采样交易明细。
---
# A股盘前短线标的筛选 v6.29.6

## 版本历史
- **v6.29.7**: 回测冠军独立计算: is_champion(预测冠军)与backtest_champion(回测冠军)分离; 回测按运行日实际收益率确定冠军标的并写回推荐历史; _rep_rank改用return_pct排序; 新增backtest_champion_codes返回集

- **v6.29.6**: md-to-html 补齐: 推荐标的table手动解析渲染+冠军卡片+章节导航+全景图嵌入+买入池注解; 同步更新所有版本锚点

- **v6.29.5**: 买入池展示优化: markdown报告只展示经过期望过滤的买入池标的, 隐藏未通过标的; 新增买入池摘要行(候选N只→买入池M只)

- **v6.29.4**: 步骤28自动整改去重守卫: 同整改内容已在近期记录中存在时跳过, 避免每天重复触发版本号递增

- **v6.29.3**: 主力资金代理估算增强: 新增换手率+振幅辅助因子加权估算

- **v6.29.2**: 主力资金代理估算增强: 新增换手率+振幅辅助因子加权估算

- **v6.29.1**: 主力资金代理估算增强: 新增换手率+振幅辅助因子加权估算

- **v6.29.0**: 影子策略自动解禁: 禁用策略影子样本回测胜率>=shadow_release_winrate(50%)且样本>=shadow_release_min_trades(5)即自动移出禁用集合恢复正常参与, 状态持久化至策略禁启用状态.json

- **v6.28.0**: 影子追踪机制: 禁用策略(G/I等)匹配标的由step13路由入_shadow_pool, 不入正式推荐/买入池

- **v6.27.0**: 新增策略V(业绩预告跳空): 财报季月份(1/3/4/8/10)+涨2-7%+量比>=1.2+收阳+非亏损, 补齐C事件驱动单薄

- **v6.26.0**: 行业负面清单补全: 新增医药生物(15.4%)/计算机(10%)/环保(10%)/交通运输(14.3%)四行业, 剔除后预期整体胜率从28.1%→35%+

- **v6.25.0**: 止损全线收窄至3.5-4% + 行业负面清单(剔除8弱行业)提升胜率

- **v6.24.1**: 修复: 皇冠标的被买入池期望过滤剔除, 漏写推荐历史; v6.24.1 皇冠标的豁免期望过滤: 负期望策略D的当日👑冠军不再被剔除, 保证写入推荐历史供回测追踪

- **v6.24.0**: 取消策略A筛选限制: 弱市不再关闭(open) + 移除震荡市数量上限(9999=不限); 回测胜率低策略仅不参与皇冠评选(crown_min_strategy_winrate门槛), 不再自动收紧限流参数

- **v6.23.1**: 修复策略A震荡市上限守卫条件: sa_limit>=0(原>0使0=不限而非完全关闭), 与步骤28'上限已达最低0'冲突; 一并修正7处锚点同步

- **v6.23.0**: 历史回测盈利加成(bt_bonus)连续化评分: 胜率连续加成(0~0.7)+均收精细化(0~0.3)+置信度加权(0~1.0)+衰减(0.5)→连续区间0~0.5; 样本门槛≥2→≥1(低样由置信度压制); school rounding替代banker's rounding消除显示失真; 板块加成同规整
