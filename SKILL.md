...
当 clist API 或新浪 API 无法返回申万行业分类时，使用以下基于代码段的行业查表。查表基于上证/深证代码分配惯例与申万一级行业映射，覆盖约 85% 的标的。查表无法覆盖的码段统一标记为「未知」而非板块名。

```python
# 代码段 → 申万一级行业查表 (v6.6.23)
INDUSTRY_MAP = {
    # 上海主板 600xxx
...
# 查表逻辑：code_int = int(code)；遍历区间，落在区间内即返回对应行业
# 查表无法覆盖 → industry = "未知"（绝不填入板块名）
```

**变更理由**：v6.6.22 及之前版本在行业不可得时以板块（深圳主板/上海主板）作为兜底值，导致行业列显示的是交易所板块而非申万行业。v6.6.23 改用代码段查表，查不到时标记「未知」，彻底杜绝分类学错误。
