...
description: A股每日盘前短线标的智能筛选(v6.6.29)。基于前一日收盘数据，通过 35步筛选流程（网络授时北京时间→GitHub拉取持仓跟踪→节假日检查→极端行情→外围市场→持仓同步→做T评估→持仓跟踪同步→持仓危机检查→全市场API拉取(腾讯qt.gtimg.cn一级→新浪hq.sinajs.cn二级→pytdx三级降级)→行业查表补全(210+代码段全覆盖+硬编码精准修正)→31项硬排除(L1/L2/L3三级可达性)→14项信号过滤→五大策略评分（含评分相同时二次评估打破平局+策略进场价计算）→行业集中度→新闻筛查→生成HTML报告(含三大指数实时行情)→GitHub同步(含超15天旧文件自动清理)→飞书推送→每周复盘+数据源监控告警(连续10次腾讯一级不可达则直达用户），输出短线标的_YYYYMMDD.md 预测次日上涨的标的为Markdown格式，同时生成可视化HTML报告。推荐历史按日期归档(推荐历史_YYYYMMDD.json)互不覆盖，告警日志仅在自动化中写。当用户需要运行盘前筛选、A股短线选股、每日标的预测时使用。
---
# A股盘前短线标的筛选 v6.6.29
...
**优先级2 — 代码段查表**（v6.6.29，210+代码段覆盖全部A股：600000-609999 / 000001-005999 / 300000-304999）：优先级1未命中时执行。

```python
# 代码段 → 申万一级行业查表 (v6.6.29 — 全A股覆盖)
INDUSTRY_MAP = {
    # 上海主板 600xxx-609xxx
