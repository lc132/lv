...
description: A股每日盘前短线标的智能筛选(v6.12.3)。基于前一日收盘数据，通过35步筛选流程（网络授时北京→GitHub拉取行业缓存+验证+节假日→极端行情→外围市场→持仓同步→做T→持仓跟踪→持仓危机→全市场API拉取→行业缓存读取(一级+二级)→pytdx历史K线→东方财富财务(已降级)→F10单股API→风险事件→拥挤度→13项硬排除→27项信号过滤→二十策略评分+MACD/K线技术指标+多因子共振→微观结构过滤→行业限制→新闻筛查→TOP10龙虎榜采集+正面新闻筛查+公司公告→行业集中度→AI策略分析(TOP10精选+市场全景+板块研判+个股深度研判)→生成HTML报告（含TOP10精选推荐理由+AI分析）→GitHub同步→飞书推送→每周复盘），输出短线标的_YYYYMMDD.md 为Markdown格式，同时生成可视化HTML报告。v6.12.2：修复AI分析数据源+版本号统一——AI分析从中间池(102只)移至最终TOP10池，确保个股深度研报与推荐表完全一致；全仓库BUILTIN_VERSION统一为v6.12.3。
---
# A股盘前短线标的筛选 v6.12.3

## 版本历史
- **v6.12.3**: 行业分类修正——行业缓存为空时代码段映射错误率极高，补充23只HARDCODED_INDUSTRY修正（银轮股份/金字火腿/东材科技/英维克/九安医疗/禾望电气/密尔克卫/华安证券/中国巨石等），全仓库BUILTIN_VERSION统一为v6.12.3
- **v6.12.2**: 修复AI分析数据源——步骤15B从中间池(102只)移至最终TOP10池(步骤19后)，修复前AI分析了102只中间候选(含后续被过滤标的如立讯精密/华天科技等)，修复后仅分析最终精选TOP10按盈亏比排序的标的；全仓库BUILTIN_VERSION统一为v6.12.3(ashare_screener.py/lib/core.py/SKILL.md/_meta.json)
