...
2. 筛选管道（6级漏斗数量）
3. 推荐标的表格（# | 策略 | 标的 | 代码 | 行业 | 涨跌幅 | 开盘 | 收盘 | 振幅 | 7日 | 评分 | 置信 | 进场 | 止损 | 止盈）
4. 策略分布统计 + 策略说明
5. 硬排除 TOP5
6. 免责声明
...
- **步骤20B 生成HTML报告**（必须完整生成，不可省略任何组件）：基于筛选结果生成自包含HTML报告，输出到 `/workspace/ashare-screening-YYYYMMDD/` 目录。HTML报告与Markdown同步推送至GitHub。报告必须包含以下 **7个区域**（缺一不可）：
  ...
  4. **最终推荐标的表**（14列）：序号|策略|标的|代码|行业|涨跌幅|开盘价|收盘价|振幅|7日|评分|置信度|进场|止损|止盈。7日列标记近7日已推荐标的（★），行背景橙色高亮（`recent-7d` CSS类）。策略用彩色badge，涨跌幅红涨绿跌，置信度★★★绿/★★黄/★红，标的名称+代码超链接到东方财富个股页，行背景色按策略着色。
