...
description: A股每日盘前短线标的智能筛选(v6.4.16)。基于前一日收盘数据，通过 33步筛选流程（网络授时北京时间→节假日检查→极端行情→外围市场→持仓同步→做T评估→持仓跟踪同步→持仓危机检查→31项硬排除(L1/L2/L3三级可达性)→14项信号过滤→五大策略评分→行业集中度→新闻筛查→GitHub同步→飞书推送→每周复盘），仅输出短线标的_YYYYMMDD.xlsx 预测次日上涨的标的到Excel。推荐历史json和告警日志仅在自动化中写。当用户需要运行盘前筛选、A股短线选股、每日标的预测时使用。
---
# A股盘前短线标的筛选 v6.4.16
...
**6.文件初始化**：策略调整记录.json取末条version+params，损坏→默认v6.4.13。交叉验证推荐历史中strategy_check版本，不一致以策略调整记录为准→log_alert INFO。**首次运行或版本变更→safe_append_json追加type="strategy_check"记录**（含version/params/checks），验证各项条件计数与预期一致
...
def fetch_stock_quote(code, name, data_date):
    """通过定向URL获取精确行情，返回 dict 或 None"""
    market = 'sz' if code.startswith(('000','002','003','300','301')) else 'sh'
    
    # 方案一：新浪财经实时/历史行情API（优先，最可靠）
    try:
        ...
        # 解析格式：var hq_str_xxx="名称,开盘,昨收,现价,最高,最低,..."
        if text and '=""' not in text:
            parts = text.split('"')[1].split(',')
            if len(parts) >= 32:
                open_price = float(parts[1]) if parts[1] else None
                close_price = float(parts[2]) if parts[2] else None  # 昨收
                current = float(parts[3]) if parts[3] else None
                high = float(parts[4]) if parts[4] else None
                low = float(parts[5]) if parts[5] else None
                # 新浪接口返回实时数据；若非交易日，现价≈前日收盘
                if open_price is None or open_price <= 0:
                    raise ValueError("价格数据无效")
                amplitude = round((high - low) / close_price * 100, 2) if close_price else None
                change_pct = round((current - close_price) / close_price * 100, 2) if current and close_price else None
                # 换手率：新浪格式中 index 38
                turnover = float(parts[37]) if len(parts) > 37 and parts[37] else None
                return {
                    "open": open_price, "close": current if current else close_price,
    ...
    # 方案二：东方财富行情页（WebFetch解析）
    try:
        eastmoney_url = f'https://push2.eastmoney.com/api/qt/stock/get?secid={market}.{market}{code}&fields=f43,f44,f45,f46,f47,f48,f50,f57,f58,f60,f168,f169,f170'
        req = urllib.request.Request(eastmoney_url, headers={'User-Agent': 'Mozilla/5.0'})
        resp = urllib.request.urlopen(req, timeout=5)
        data = json.loads(resp.read())
        if data.get('data'):
            d = data['data']
            ...
            if open_price and open_price > 0:
                return {
                    ...
                    "amplitude": amplitude, "change_pct": change_pct, "source": "eastmoney"
                }
    except Exception:
        pass

    # 方案三：降级到WebSearch（仅在前两个API失败时）
    # 搜索后必须验证返回数据的日期匹配 data_date
    return None  # 三个方案均失败 → open/close/turnover/amplitude 填 None

# === 数据校验（关键新增） ===
def validate_quote(quote, code, name):
    """校验行情数据合理性，不通过则标记为不可用"""
    if quote is None:
        return None
...
对每只通过硬排除的标的调用：
```python
quote = fetch_stock_quote(code, name, data_date)
quote = validate_quote(quote, code, name)
if quote:
