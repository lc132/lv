...
description: A股每日盘前短线标的智能筛选(v6.12.0)。基于前一日收盘数据，通过35步筛选流程（网络授时北京→GitHub拉取行业缓存+验证+节假日→极端行情→外围市场→持仓同步→做T→持仓跟踪→持仓危机→全市场API拉取→行业缓存读取(一级+二级)→pytdx历史K线→东方财富财务(已降级)→F10单股API→风险事件→拥挤度→13项硬排除→27项信号过滤→二十策略评分+MACD/K线技术指标+多因子共振→微观结构过滤→AI策略分析(市场全景+板块研判+个股深度研判)→TOP10龙虎榜采集+正面新闻筛查+公司公告→行业集中度→生成HTML报告（含TOP10精选推荐理由+AI分析）→GitHub同步→飞书推送→每周复盘），输出短线标的_YYYYMMDD.md 为Markdown格式，同时生成可视化HTML报告。v6.12.0：将单纯数据筛选升级为AI智能分析(lib/analyst.py)——大模型充当专属策略分析师，对每个TOP10标的生成策略逻辑/技术面/资金面/基本面/风险提示/操作建议/综合研判七维深度分析，附市场全景和板块研判。
---
# A股盘前短线标的筛选 v6.12.0
...
- 步骤15B: AI 策略分析（市场全景+板块研判+个股深度研判，v6.12.0新增）
- 步骤16: 综合评分+平局打破
- 步骤17: 行业限制
- 步骤18: 新闻筛查
- 步骤19: 降级
