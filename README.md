...
基于**前一日收盘数据**，通过 37 步筛选流程 + 步骤28 自动整改 + 策略级胜率监控熔断，自动输出每日盘前短线标的清单（`短线标的_YYYY-MM-DD.md`）与可视化 HTML 报告，并通过 GitHub 与飞书同步推送。**v6.25.0 起止损全线收窄至 3.5%-4%，并启用行业负面清单（剔除 8 个回测胜率<40% 的弱势行业）以提升整体胜率**。
