...
# 策略止损/止盈比例（与主脚本 _STRATEGY_STOP_LOSS / _STRATEGY_TAKE_PROFIT 一致）v6.13.7: 同步主脚本
_STRATEGY_STOP_LOSS = {
...
def _fetch_kline_range(code, start_date, lmt=15):
    ...
    # 二级降级: iTick API
    itick_key = os.environ.get("ITICK_API_KEY", "")  # v6.13.7: 移除硬编码默认值
    if itick_key:
        try:
...
def _compute_metrics(trades):
    ...
    # v6.13.7: 最大回撤从峰值计算（而非累计重置），更准确反映风险
    ...
    if len(returns) > 1:
        ...
        variance = sum((r - avg_r) ** 2 for r in returns) / (len(returns) - 1)  # v6.13.7: 样本方差N-1
        std = variance ** 0.5
...
def run_backtest(hold_days=10, max_days_lookback=90):
    ...
    today = datetime.now() + timedelta(hours=8)  # v6.13.7: 北京时间（与主脚本一致）
    ...
    # v6.13.7: 去重key改为(code,date,strategy,entry)，保留同股票不同策略的推荐
    seen = set()
    unique_history = []
    for h in history:
    ...
    code_kline_cache = {}
    # v6.13.7: 按 (code, prediction_date) 分别拉取K线，而非仅取最新pred_date
...
def generate_backtest_report(bt_result, output_path=None):
    ...
    today_str = (datetime.now() + timedelta(hours=8)).strftime('%Y-%m-%d')  # v6.13.7: 北京时间
    lines = [
        f"# A股短线筛选 — 历史回测报告",
        f"",
        f"- **生成日期**: {today_str}",
...
<body><h1>历史回测报告</h1><p>暂无回测数据。</p><h2>回测说明</h2><ul><li>回测使用最近90天推荐历史。</li><li>单笔最大持仓10个交易日。</li><li>按推荐时的进场、止损、止盈价格进行模拟。</li><li>遵循A股T+1规则，买入当日不检查止盈止损出场。</li><li>回测未计入滑点、手续费、涨跌停无法成交、真实排队成交等因素，仅供参考。</li></ul><p style="color:#94a3b8">版本: v6.13.7 | 生成: {today_str}</p></body></html>'''
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(html)
        return output_path
