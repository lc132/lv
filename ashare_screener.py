...
_STRATEGY_STOP_LOSS = {'A': 0.94, 'B': 0.93, 'C': 0.94, 'D': 0.94, 'E': 0.96, 'F': 0.95, 'G': 0.93, 'H': 0.94, 'I': 0.93, 'J': 0.94, 'K': 0.94, 'L': 0.94, 'M': 0.945, 'N': 0.94, 'O': 0.95, 'P': 0.945, 'Q': 0.95, 'R': 0.95, 'S': 0.95, 'T': 0.94, 'U': 0.93}
...
def _git_with_token(cmd_args, timeout=30, check=True, log_prefix=""):
    ...
    if GITHUB_TOKEN:
        # 方案1: 尝试 GIT_ASKPASS（更安全，Token 不会出现在进程列表/日志中）
        ...
        try:
            ...
            try:
                ...
                pass
        # 方案2: 降级为 Token-in-URL（沙箱兼容方案，Token 可能出现在进程列表中）
        new_args = []
        is_push = any('push' in str(a) for a in cmd_args)
        for arg in cmd_args:
            if arg.startswith("https://github.com/") and "@github.com" not in arg:
                arg = f"https://{GITHUB_TOKEN}@github.com/{arg.split('github.com/', 1)[1]}"
            if arg == "origin" and is_push:
...
def step0B_sync_industry_cache():
    ...
    本地有效则跳过拉取；缺失/损坏时从数据仓(lc132/lv-data)自动同步。
    ...
    for fname in sorted(os.listdir('/workspace')):
        ...
        for rec in records:
            if rec.get('type') != 'recommendation':
                continue
            # @since v6.22.27: 改为"回溯补采"而非"单日严格窗口"。
            # 旧逻辑仅匹配 prediction_date == today，若买入日当天盘后未重跑则该记录永久
            # 丢失采集(real_return_pct 全空的根因)。现改为：买入日交易日已到(<= today)
            # 且尚未采集即回溯拉取 prediction_date 当日K线补齐，历史缺失可在下次运行补上。
            pdate = rec.get('prediction_date', '')
            if not pdate:
                continue
            # 幂等：判 real_return_pct 而非 real_confirm_date(兼容0值有效采集)
            if rec.get('real_return_pct') is not None or rec.get('real_confirm_date'):
                continue
            # 仅采集交易日已到(含当日)的记录；未来预测日(尚未到买入日)跳过
            if pdate > today:
                continue
            ...
            # 回溯拉取 prediction_date 当日K线(腾讯HTTP)
            kline = _fetch_single_kline(code, pdate)
            if not kline:
                log_alert("DEBUG", "真实收益", f"{code} {rec.get('name','')} {pdate}K线获取失败，跳过")
                continue
...
HARDCODED_INDUSTRY = {
    ...
    '605208': '汽车',      # 永茂泰（汽车铝合金零部件，在605200-605299段但非基础化工）
    '000034': '计算机',    # 神州数码（IT分销/云服务，在000000-000099段但非银行）
    '603698': '国防军工',  # 航天工程（航天煤化工工程，在603600-603699段但非轻工制造）
    '000893': '基础化工',  # 亚钾国际（钾肥开采，在000800-000899段但非汽车）
    '002179': '电子',      # 中航光电（军工电子连接器，在002100-002199段但非医药生物）
    '600580': '电力设备',  # 卧龙电驱（电机/电控，在600500-600599段但非食品饮料）
    '002402': '电子',      # 和而泰（智能控制器，在002400-002499段但非传媒）
    '002273': '电子',      # 水晶光电（光学元器件，在002200-002299段但非建筑装饰）
    '603296': '电子',      # 华勤技术（ODM/电子制造，在603200-603299段但非基础化工）
    '600602': '计算机',    # 云赛智联（智慧城市/IT服务，在600600-600699段但非食品饮料）
    '600862': '国防军工',  # 中航高科（航空复合材料，在600800-600899段但非煤炭）
    '603160': '电子',      # 汇顶科技（指纹识别芯片，在603100-603199段但非机械设备）
    '002249': '电力设备',  # 大洋电机（微特电机/新能源车动力总成，在002200-002299段但非建筑装饰）
    '603638': '机械设备',  # 艾迪精密（液压破碎锤，在603600-603699段但非轻工制造）
    '605111': '电子',      # 新洁能（功率半导体/MOSFET，在605100-605199段但非电力设备）
    '002317': '医药生物',  # 众生药业（中成药/化药，在002300-002399段但非电力设备）
    '603067': '基础化工',  # 振华股份（铬盐/化工，在603000-603099段但非电子）
    '002245': '电力设备',  # 蔚蓝锂芯（锂电池/金属物流，在002200-002299段但非建筑装饰）
    '600531': '有色金属',  # 豫光金铅（铅锌冶炼，在600500-600599段但非食品饮料）
    '002119': '电子',      # 康强电子（引线框架/半导体封装材料，在002100-002199段但非医药生物）
    '601609': '有色金属',  # 金田股份（铜加工/铜合金，在601600-601699段但非煤炭）
...
def _fetch_single_kline_eastmoney(c):
    ...
    try:
        ...
        bars = []
        try:
            turl = (f'https://proxy.finance.qq.com/ifzqgtimg/appstock/app/fqkline/get?'
        ...
        return {
            ...
            'high60': max(highs[-60:]) if len(highs) >= 60 else (max(highs) if highs else 0),
            'low60': min(lows[-60:]) if len(lows) >= 60 else (min(lows) if lows else 0),
            'days_listed': len(bars) if bars else 999, 'limit_up_days': 0,
            'closes': closes, 'highs': highs, 'lows': lows, 'volumes': volumes
        }
    except (urllib.error.URLError, json.JSONDecodeError, OSError,
            ValueError, TypeError, ZeroDivisionError, IndexError):
...
def _filter_buy_pool(candidates, strategy_metrics=None):
    ...
    达标标的不足时自然少买(不硬凑)。不再设硬上限, 避免冠军标的中途被截断。
...
def main():
    ...
    print("\n[步骤0C] 真实收益采集(回溯补采)..."); step0C_collect_real_returns()
    record_step_status("步骤0C: 真实收益采集", "OK")

    # @since v6.22.27: 防重短路置于 step0C 之后——当日报告已存在时不再 sys.exit(0)
    # 跳过补采，保证 real_return_pct 缺失历史得以回溯补齐。
    print("\n[步骤0D] day-level防重(报告已存在则退出)..."); step0D_day_dedup()
